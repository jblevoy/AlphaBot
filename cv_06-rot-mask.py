...
field = cv.imread('/Users/jasen_levoy/Documents/Projects/PlotBot/PicknPlace/alpha_blocks_ab.jpg',1) #30 deg and -15 deg 2 deg white background
...
a = cv.imread("alpha_blocks_a-0.jpg", 1)
a = cv.pyrDown(a)
a_gs = cv.cvtColor(a, cv.COLOR_BGR2GRAY)  
th_val, a_inv = cv.threshold(a_gs, 180, 255, cv.THRESH_BINARY_INV)  #a_inv = 'a' template threshold + inverse
...
b = cv.imread("alpha_blocks_b-0.jpg", 1)
b = cv.pyrDown(b)
