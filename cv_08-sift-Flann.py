...
# changed min match count from 10 to 5
MIN_MATCH_COUNT = 10

path = "/Users/jasen_levoy/Documents/Projects/PlotBot/PicknPlace/"

query_image_fn = path+"alpha_blocks_a-0.png"  #query = template
train_image_fn = path+"alpha_blocks_ab.png"  #train = field
...
for i in range(n_clusters_):
    ...
    if len(good)>MIN_MATCH_COUNT:
        ...
        if M is None:
            ...
            img_inc += 1
            #cv2.imwrite('Block_Matches_inc{}.png'.format(img_inc), img3)
            #cv2.imwrite('Block_Matches.png'.format(img_inc), img2)
            #print(dst)
            print('x,y: ',(dst[0][0][0]+dst[2][0][0])/2,(dst[0][0][1]+dst[2][0][1])/2)
            print()
